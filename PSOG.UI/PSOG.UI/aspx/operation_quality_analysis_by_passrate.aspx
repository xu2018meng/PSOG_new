<%@ Page Language="C#" AutoEventWireup="true" CodeFile="operation_quality_analysis_by_passrate.aspx.cs" Inherits="operation_quality_analysis_by_passrate" %>
...
<body  class="easyui-layout" style="width:100%; height:100%;">
        ...
        <iframe id="hidden_frame" name="hidden_frame" style="display:none"></iframe>
            <table class="tblSearch" style="text-align:left">
                <tr style="height:auto;">                        
                    ...
                    指标查询：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              <div style="padding:5px 0;display:inline"><a id="passrate_process" title="工段-操作合格率分布。" style=" border:none" class="easyui-linkbutton easyui-tooltip" data-options="iconCls:'icon-distribution',size:'large',iconAlign:'top'"><b>工段分布</b></a></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              <div style="padding:5px 0;display:inline"><a id="passrate_class" title="班组-操作合格率分布。" style=" border:none" class="easyui-linkbutton easyui-tooltip" data-options="iconCls:'icon-distribution',size:'large',iconAlign:'top'"><b>班组分布</b></a></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              <div style="padding:5px 0;display:inline"><a id="passrate_time" title="时间-操作合格率分布。" style=" border:none" class="easyui-linkbutton easyui-tooltip" data-options="iconCls:'icon-distribution',size:'large',iconAlign:'top'"><b>时间分布</b></a></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    </td>
                </tr>
            </table> 
        ...
        <table width="100%" height="100%">
            <tr>
                ...
                <td style="border-width:1px; width:80%">
                    ...
                    <div id="container_passrate_process" style="height: 500px; margin: 0 auto; display:block">
					</div>
                    
                    <div id="container_passrate_class" style="height: 500px; margin: 0 auto; display:none">
                    </div>
					
					<div id="container_passrate_time" style="height: 500px; margin: 0 auto; display:none">
					</div>
    ...
    $(function(){
        ...
        show_passrate_process();
        
        $('#passrate_process').click(function (){   //操作合格率工段分布情况
		    $('#container_passrate_process').show();
            $('#container_passrate_class').hide();
            $('#container_passrate_time').hide();
            
            show_passrate_process();
		});
		
		$('#passrate_time').click(function (){   //操作合格率时间分布情况
		    $('#container_passrate_process').hide();
            $('#container_passrate_class').hide();
            $('#container_passrate_time').show();
            
            show_passrate_time();
		});
		
		$('#passrate_class').click(function (){   //操作合格率班组分布情况
		    $('#container_passrate_process').hide();
            $('#container_passrate_class').show();
            $('#container_passrate_time').hide();
		...
		function show_passrate_process(){   //执行操作合格率工段分布功能
		    //数据通过ajax实现
...
//            }
            ...
            if(flag == 0){
                width_temp = document.getElementById("container_passrate_process").offsetWidth;
                flag = 1;    
            }
            
            $('#container_passrate_process').highcharts({
		        chart: {
			        type: 'column',
			        width: width_temp
		        },
		        title: {
			        useHTML: true,
                    text: '<span style="font-size: large; color:Black; font-weight:bold">操作合格率工段分布情况</span> <img id="tip_passrate_process" title="" src="../resource/img/question.png" style="cursor:pointer;"/>'
		        },
		        navigation: {
                    buttonOptions: {
                        enabled: false
...
//                    data: [10, 5, 15, 21]
            ...
            var chart = $('#container_passrate_process').highcharts();
            chart.showLoading('正在加载数据...');
            
            $.ajax({
                type: "post",
                url: 'operation_quality_analysis_by_passrate.ashx?id=1&plantId=<%=plantId %>&startTime='+startTime+'&endTime='+endTime,
                data: null,
                dataType: "json",
                success: function(data){
                    var array = [];  
		...
		function show_passrate_class(){   //执行操作合格率班组分布功能
		    //数据通过ajax实现
		    var startTime = $("#startTime").val();
            var endTime = $("#endTime").val();
...
//            }
            ...
            $('#container_passrate_class').highcharts({
		        chart: {
			        type: 'column',
			        width: width_temp
		        },
		        title: {
			        useHTML: true,
                    text: '<span style="font-size: large; color:Black; font-weight:bold">操作合格率班组分布情况</span> <img id="tip_passrate_class" title="" src="../resource/img/question.png" style="cursor:pointer;"/>'
		        },
		        subtitle: {
                    text: sub_title
                },
...
//                    data: [10, 5, 15, 21]
            ...
            var chart = $('#container_passrate_class').highcharts();
            chart.showLoading('正在加载数据...');
            $.ajax({
                type: "post",
                url: 'operation_quality_analysis_by_passrate.ashx?id=2&plantId=<%=plantId %>&startTime='+startTime+'&endTime='+endTime,
                data: null,
                dataType: "json",
                success: function(data){
                    var array = [];  
		...
		function show_passrate_time(){   //执行操作合格率时间分布功能
		    //数据通过ajax实现
		    var startTime = $("#startTime").val();
            var endTime = $("#endTime").val();
...
//            }
            ...
            $('#container_passrate_time').highcharts({
				chart: {
					zoomType: 'x',
					width: width_temp
				},
				title: {
					useHTML: true,
                    text: '<span style="font-size: large; color:Black; font-weight:bold">操作合格率时间分布</span> <img id="tip_passrate_time" title="" src="../resource/img/question.png" style="cursor:pointer;"/>'
				},
				subtitle: {
                    text: sub_title
                },
...
//					data: [10, 12, 13, 10, 15, 50, 35, 10]
            ...
            var chart = $('#container_passrate_time').highcharts();
            chart.showLoading('正在加载数据...');
            
            $.ajax({
                type: "post",
                url: 'operation_quality_analysis_by_passrate_time.ashx?plantId=<%=plantId %>&startTime='+startTime+'&endTime='+endTime,
                data: null,
                dataType: "json",
                success: function(data){
                    var new_data = [];
